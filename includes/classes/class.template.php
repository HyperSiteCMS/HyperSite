...
class template {
     ...
     * @access public
     */
    function __construct($template_path, $template_name, $fallback_template_path = false) {
        global $root_path;
        if (substr($template_path, -1) == '/') {
            $template_path = substr($template_path, 0, -1);
        }
        $this->root = $template_path;
        $this->cachepath = $root_path . 'cache/ctpl_' . str_replace('_', '-', $template_name) . '_';
    ...
    function set_template($template_path, $template_name, $fallback_template_path = false) {
        global $root_path;
    ...
    function set_filenames($filename_array) {
        ...
        foreach ($filename_array as $handle => $filename) {
            if (empty($filename)) {
                die("template->set_filenames: Empty filename specified for $handle");
            }
    ...
    function _tpl_load(&$handle) {
        ...
        if (!isset($this->filename[$handle])) {
            die("template->_tpl_load(): No file specified for handle $handle");
        }
...
class template_compile {
    ...
    function _tpl_load_file($handle) {
        ...
        if (!file_exists($this->template->files[$handle])) {
            die("template->_tpl_load_file(): File {$this->template->files[$handle]} does not exist or is empty");
        }
